...
{
	cuCheck ( cuModuleLoad ( &m_Module, CUDA_FLUID_PTX_PATH), "LoadKernel", "cuModuleLoad", CUDA_FLUID_PTX_PATH, mbDebug);
...
const char* FluidSystem::DrawParticleInfo ( int p )
{
	...
	Vector3DF pos = m_Fluid.bufV3(FPOS)[p];
	sprintf ( disp, "Position: <%.2f, %.2f, %.2f>", pos.x, pos.y, pos.z );		drawText( 10, 90, disp, 0, 0, 0, 1 );

	for (int i = 0; i < mNumPoints; i++)
	{
		Vector3DF vec = m_Fluid.bufV3(FPOS)[i];

		if (i == 0 || i == mNumPoints - 1)
		{
			char buf[6];
			sprintf(buf, "%d\t%.2f\t", i, vec.x);
			nvprintf(buf);
		}
	}
	nvprintf("\n");
